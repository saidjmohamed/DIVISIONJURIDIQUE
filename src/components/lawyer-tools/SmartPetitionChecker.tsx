        ...
        <div className="space-y-4">
              ...
              <div className={`rounded-xl p-4 border ${v.bg}`}>
                ...
                {analysis.aiPowered && analysis.modelLabel && (
                  <div className="mt-2 flex items-center gap-1.5 text-[9px] text-gray-500">
                    <span>🤖</span>
                    <span>التحليل بواسطة: {analysis.modelLabel}</span>
                    {analysis.triedModels && <span className="text-gray-400">({analysis.triedModels.length} نموذج جُرّبت)</span>}
                  </div>
                )}
              </div>
            );
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-1">
            {[
              ...
              { key: 'passed' as const, label: `✅ مستوفى (${analysis.passedChecks.length})` },
              { key: 'failed' as const, label: `❌ غير مستوفى (${analysis.failedChecks.length})` },
              { key: 'pending' as const, label: `🔍 معلّق (${analysis.pendingChecks.length})` },
              { key: 'suggestions' as const, label: `✏️ اقتراحات (${analysis.suggestions.length})` },
            ].map(tab => (
              <button
                key={tab.key}
                onClick={() => setFilterTab(tab.key)}
                className={`text-[10px] px-3 py-1.5 rounded-full transition-all font-medium whitespace-nowrap ${
                  filterTab === tab.key ? 'bg-[#7c3aed] text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400'
                }`}
              >
                {tab.label}
              </button>
          ...
          {(filterTab === 'all' || filterTab === 'pending') && analysis.pendingChecks.length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
              <h4 className="text-sm font-bold text-blue-700 dark:text-blue-400 mb-3">🔍 فحوص معلّقة ({analysis.pendingChecks.length})</h4>
              <div className="space-y-2">
          ...
          <div className="bg-amber-50 dark:bg-amber-900/15 border border-amber-200 dark:border-amber-800 rounded-xl p-3">
            <p className="text-[10px] text-amber-700 dark:text-amber-400 leading-relaxed">
              ⚠️ <strong>تنبيه قانوني وإخلاء مسؤولية:</strong> هذه الأداة مخصصة للفحص الشكلي الأولي للعرائض والمذكرات، ولا تغني بأي حال عن مراجعة المحامي أو المستشار القانوني المختص.
            </p>
          </div>
